...
$ToPythonStrings::usage="Specifies what the compilation target is for ToPython"


BeginPackage["`Package`"];


PyRow::usage="Linear formatted python code";
PyBlock::usage="Block formatted python code";


PyStructure::usage=
  "Specifies a direct conversion of a structure to be called with ToPython";
PyAlias::usage=
  "Generates an alias for a lower-level form to be called with ToPython";


$PyStructures::usage=
  "A collection of structures that represent python symbolically";
$PyAliases::usage=
  "A collection of structures that have further possible reductions";
...
If[!ValueQ[$ToPythonStrings], $ToPythonStrings=False];
IfPS[a_, b_]:=
...
With[{$cont=$Context},
PyStructure/:
  HoldPattern[(h:Set|SetDelayed)[PyStructure[a_], v_]]:=
    With[{sym=FirstCase[Hold[a],_Symbol,None,{2,\[Infinity]},Heads->True]},
      If[sym=!=None,
        If[Context[sym]==$cont,
          Context[sym]=Context@PyStructure
          ];
...
PyStructure[PyImportSow[i_]]:=
  Block[{$PythonInCall=True},
    PyImportSow@i
    ];
PyStructure[PyImportAsSow[import_,as_]]:=
  Block[{$PythonInCall=True},
    PyImportAsSow[import,as]
    ];
PyStructure[PyFromImportSow[from_,import_]]:=
  Block[{$PythonInCall=True},
    PyFromImportSow[from,import]
    ];
PyStructure[PyFromImportAsSow[from_,import_,as_]]:=
  Block[{$PythonInCall=True},
...
PyStructure[PyInt[i_?NumericQ]]:=
  IfPS[toNumberString@IntegerPart[i], i];
PyStructure[PyFloat[n_?NumericQ,prec:_Integer|Automatic:Automatic]]:=
  IfPS[toNumberString@If[prec===Automatic,N[n],N[n,prec]], n];
PyStructure@PyVerbatim[v_]:=
  IfPS[ToString[v], v];
...
PyStructure@PyBoolean[t:True]:=
  IfPS["True", t]
PyStructure@PyBoolean[f:False]:=
  IfPS["False", f];
PyAlias[PyTrue]:=
...
$PyRuleChar="=";
PyStructure@PyRule[a_,b_,rs:_String|Automatic:Automatic]:=
  IfPS[
...
PyStructure[PyDict[l_List]]:=
  IfPS[
    StringJoin@{"{ ",Riffle[Block[{$PyRuleChar=":"},ToPython/@Normal@l],", "]," }"},
    ToPython@PyCall["dict", List@@@l]
    ];
PyStructure[PyDict[a_Association]]:=
...
PyStructure[PySequence[args_List]]:=
  IfPS[
...
PyStructure[s_String]:=
  s;
PyStructure[s_Symbol]:=
  ToPython@PySymbol[s];
PyStructure[i_Integer]:=
  ToPython@PyInt[i];
PyStructure[r_Real]:=
  ToPython@PyFloat[r];
PyStructure[t:True|False]:=
  ToPython@PyBoolean[t];
PyStructure[r:_Rule|_RuleDelayed]:=
  ToPython@PyRule[r];
PyStructure[r:_Association|{(_Rule|_RuleDelayed)..}]:=
  ToPython@PyDict[r];
PyStructure[l_List]:=
  ToPython@PyList[l];


PyStructure[n_?NumericQ]:=
  n//.{
...
PyStructure[Slot[n_]]:=
  "pyArg_"<>ToString[n];
PyStructure[SlotSequence[]]:=
  "pyArgTuple"
PyStructure[SlotSequence[n_]]:=
  "pyArgTuple[n:]"
...
PyStructure@p:PyRow[args_List,riffle:_String:""]:=
  IfPS[
...
PyStructure@b:PyBlock[
  ...
  IfPS[
    ...
    PyBlock[head, indent, separator][
      ToPython/@args
      ]
    ];
PyStructure@PyBlock[e__][a:Except[_List],b___]:=
...
$PyLangTranslations=
  {
    ...
    HoldPattern[Import[f:Except[_Rule|_Dot|_PyDot]->v_]]:>
      PyImportAs[f,v],
    HoldPattern[Import[(Dot|PyDot)[f__,v_]]]:>
      If[Length@Hold[f]>1,
        PyFromImport[PyDot[f],v],
        PyFromImport[f,v]
        ],
...
$PackageName=StringSplit[$Context, "`"][[1]];
